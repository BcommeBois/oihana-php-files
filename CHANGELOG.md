...
- Documents the ReDoS (Regular Expression Denial of Service) attack surface on oihana\files\findFiles, oihana\files\shouldExcludeFile and oihana\files\copyFilteredFiles. No code change — these functions are designed for trusted patterns (configuration, internal code). Adds `@security` notes in DocBlocks, dedicated section in wiki/{fr,en}/security.md with technical explanation + defense-in-depth options, and inline warnings in wiki/{fr,en}/files/discovery.md.
- Adds opt-in decompression-bomb protection to oihana\files\archive\tar\untar via the new `TarOption::MAX_EXTRACTED_SIZE` option. When set, the archive is pre-scanned and a `RuntimeException` is thrown **before** any file is written if the sum of the entries' uncompressed sizes exceeds the cap. Default `null` preserves the historical unbounded behaviour — backward compatible. Documented in wiki/{fr,en}/archive/untar.md, wiki/{fr,en}/security.md and wiki/{fr,en}/tips.md.
- Adds opt-in size caps on file reads to guard against OOM on untrusted inputs. New `?int $maxBytes` parameter on `oihana\files\getFileLines` and `oihana\files\requireAndMergeArrays` (per-file), and new `?int $maxInputBytes` constructor parameter on `oihana\files\openssl\OpenSSLFileEncryption` (applied at the start of `encrypt()` and `decrypt()`). When the cap is exceeded, a `RuntimeException` is thrown **before** the file is read. Default `null` preserves the historical unbounded behaviour — backward compatible. Documented in wiki/{fr,en}/files/reading.md, wiki/{fr,en}/openssl/README.md, wiki/{fr,en}/security.md and wiki/{fr,en}/tips.md.
